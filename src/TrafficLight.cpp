...
void TrafficLight::cycleThroughPhases()
{
  ...
  long nextPhase = getCurrentTimeMillis() + 4000 + rand()%2000;
  queue = std::shared_ptr<MessageQueue<TrafficLightPhase>>(new MessageQueue<TrafficLightPhase>);
  ...
  while(true){
    ...
    //  cycleThroughPhases();
      ...
      nextPhase = getCurrentTimeMillis() + 4000 + rand()%2000;
    }
    //add a wait 1 second
    std::this_thread::sleep_for(std::chrono::milliseconds(1));
  }
